	...
	function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		if(  $depth == 0 && $children > 0 ) {
			 ...
			 (strpos($page->post_title,"[dd-ph]") !== false ) ? $dtoggle = ' data-toggle="dropdown"' :  $dtoggle ='';
				...
				str_replace("[dd-ph]","",$page->post_title);
				$output .= $link_before .'<a href="' . get_permalink($page->ID) . '" id="pgi-page-item-'.$page->ID.'" class="dropdown-toggle"'.$dtoggle.'>'.apply_filters( 'the_title', $page->post_title, $page->ID ) .'<b class="caret"></b>' . $link_after . '</a>';
		}
		else if(  $depth == 1 && $children > 0) {
				$output .= $link_before .'<a tabindex="-1" href="' . get_permalink($page->ID) . '" id="pgi-page-item-'.$page->ID.'">'.apply_filters( 'the_title', $page->post_title, $page->ID ) . $link_after . '</a>';
		}
		else {
